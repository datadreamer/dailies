...
function draw() {
  ...
  if(mouseIsPressed && !keyIsPressed){
    ...
    for(var i=0; i<intersections.length; i++){
      ...
      mesh.applyGravity(intersections[i].point);
    }
  }
...
function Mesh(parent, gx, gy, w, h){
  ...
  this.applyGravity = function(clickPos){
    for(var i=0; i<this.points.length; i++){
    ...
    for(var i=0; i<id; i++){
      ...
      if(dist < this.size && dist > 0){
        ...
        if(this.connections.hasOwnProperty("L"+i)){
          ...
          this.connections["L"+i].material.color.setHSL(0.5 + colorval*0.5, 1, 0.5);
        } else {
          ...
          var colorval = ((this.size - dist) / this.size);
          line.material.opacity = colorval;
          line.material.color.setHSL(0.5 + colorval*0.5, 1, 0.5);
