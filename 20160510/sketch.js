...
function draw(){
  ...
  if(millis() - lastRelease > releaseRate){
    ...
    c = color(0);
    this.walkers.push(new Walker(xpos, ypos, rot, c));
    lastRelease = millis();
  }
...
function Walker(x, y, rot, c){
  this.pos = createVector(x, y);
  //this.pos = createVector(random(windowWidth), random(windowHeight));
  this.pospast = this.pos.copy();
  this.vec = createVector(random(1,2),0);
  this.vec.rotate(rot);
  //this.vec.rotate(random(TWO_PI));
  this.rotation = 0;
  this.rotationTightness = random(0.005, 0.1);
  // var r = random();
  // if(r < 0.33){
