...
function setup(){
  ...
  bgColor = color(random(120,180), random(180,240), 255);
}
...
function mousePressed(){
  ...
  for(var i=0; i<clouds.length; i++){
    if(clouds[i].isOver(mouseX, mouseY)){
      // destroy this cloud
      clouds[i].birth = millis() - clouds[i].lifespan;
      for(var n=0; n<clouds[i].fluffbits.length; n++){
        clouds[i].fluffbits[n].damping = 1;
        clouds[i].fluffbits[n].noiseForce = 0.2;
        clouds[i].fluffbits[n].springMult = 0;
      }
      noneclicked = false;
    }
  }
...
Cloud.prototype = {
  ...
  isOver:function(mx, my){
    for(var i=0; i<this.fluffbits.length; i++){
      if(this.fluffbits[i].isOver(mx-this.x, my-this.y)){
        return true;
      }
    }
    return false;
