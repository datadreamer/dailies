...
function setup(){
  ...
  maxRadius = windowWidth/2;
  pointCount = windowWidth;
  if(pointCount > 500){
    pointCount = 500;
  }
  for(var i=0; i<pointCount; i++){
    var r = maxRadius - (maxRadius * (i/pointCount));
    var adist = (PI*tightness) / (1 + i);
    a += adist;
...
function draw(){
  ...
  //   stroke(220 - (points[i].d * 25), 360, 360);
...
function rearrange(){
  for(var i=0; i<points.length; i++){
    ...
    var xpos = windowWidth/2 + (r * cos(a));
    var ypos = windowHeight/2 + (r * sin(a));
    points[i].startX = xpos;
    points[i].startY = ypos;
    //points[i].x = xpos; // snaps to new position
    //points[i].y = ypos;
  }
}
