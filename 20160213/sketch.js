...
function draw(){
  ...
  // stroke(20);
  // noFill();
  // ellipse(0,0,radiusLimit*2,radiusLimit*2);
  noStroke();
  blendMode(SCREEN);
...
Particle.prototype = {
  ...
  move:function(){
    ...
    if(this.pos.mag() > radiusLimit){
      ...
      // mirror vector and apply it on impact
      var angle = p5.Vector.angleBetween(this.pos, this.vec);
      this.vec.rotate(angle+PI);
      //add spin force when bounce occurs, like a real dryer
      var perpx = 0 - this.pos.y * 0.07;
      var perpy = this.pos.x * 0.07;
      this.vec.add(perpx, perpy);
    }
