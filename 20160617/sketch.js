...
function setup() {
  ...
  blobs.push(new Blob(group, radius, size*2, num, 0.6));
...
function Blob(parent, radius, size, num, hue){
  ...
  //this.mat = new THREE.PointsMaterial({color:0xffffff, blending:THREE["AdditiveBlending"], vertexColors:THREE.VertexColors, transparent:true, opacity:0.5});
  ...
  for(var i=0; i<num; i++){
    ...
    c.setHSL(hue, 1, 1);
    colors.push(c);
    this.points.push(new BlobPoint(parent, pos, radius, size, hue));
...
function BlobPoint(parent, pos, radius, size, hue){
  this.parent = parent;
  this.pos = pos;
  this.radius = radius;
  this.size = size;
  ...
  this.linemat.color.setHSL(hue, 1, 0.5);
  // create point object
    ...
    for(var i=0; i<id; i++){
      ...
      if(dist < this.size && dist > 0){
        ...
        if(this.connections.hasOwnProperty("L"+i)){
          ...
          var colorval = ((this.size - dist) / this.size);
          this.connections["L"+i].material.opacity = colorval;
          // update color based on distance
          this.connections["L"+i].material.color.setHSL(0.8 - colorval, 1, 0.5);
        } else {
