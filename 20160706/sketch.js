...
function setup() {
  ...
  light2 = new THREE.PointLight(0x0099ff, 1, 1000);
...
function Block(parent){
  ...
  var geo = new THREE.BoxGeometry(65, 800, 20);
  ...
  this.obj.rotation.set(Math.PI/2, 0 - Date.now() * 0.001, 0);
  ...
  this.update = function(){
    if(this.growing){
      ...
      if(p < 1){
        ...
        //this.obj.position.set(Math.sin(sp) * this.r, 0, Math.cos(sp) * this.r);
      } else {
        this.growing = false;
        this.shrinking = true;
        this.growStart = Date.now();
      ...
      if(p < 1){
        ...
        //this.obj.position.set(Math.sin(1-sp) * this.r, 0, Math.cos(1-sp) * this.r);
      } else {
        this.shrinking = false;
        this.dead = true;
