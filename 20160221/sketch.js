...
function createShape(x, y){
  ...
  if(r1 < 0.15){
    ...
    s.sizeTo(windowWidth/5, windowWidth/50, animationDuration/2);
    s.rotateTo(PI, animationDuration);
  } else if(r1 > 0.6 && r1 < 0.75){
    s = new Frame(windowWidth/2, windowHeight/2, colors[colorIndex]);
    s.w = 0;
    s.h = windowWidth/50;
    s.sizeTo(1, windowWidth/50, animationDuration/2);
  } else if(r1 > 0.75 && r1 < 0.9){
    var w = int(random(words.length));
    if(w >= words.length){
      w = words.length-1;
    }
    s = new Word(x, y, colors[colorIndex], words[w]);
    s.a = 0;
    s.fadeTo(255, animationDuration/2);
  } else {
...
Frame.prototype.draw = function(){
  ...
  fill(red(this.c), green(this.c), blue(this.c), this.a);
  push();
  var wide = (windowWidth-(windowWidth/10));
  var high = (windowHeight-(windowWidth/10));
  var xoffset = wide/2;
  var yoffset = high/2;
  var hypo = sqrt(wide*wide + high*high);
  var hypoangle = atan2(high, wide);
  wide += this.h;
  high += this.h;
  translate(this.x, this.y);
  rect(0, 0-yoffset, this.w*wide, this.h);
  rect(0-xoffset, 0, this.h, this.w*high);
  rect(0, yoffset, this.w*wide, this.h);
  rect(xoffset, 0, this.h, this.w*high);
  push();
  rotate(hypoangle);
...
function Word(x, y, c, value){
  Shape.call(this, x, y, c);
  this.value = value;
}

Word.prototype = Object.create(Shape.prototype);
Word.prototype.constructor = Shape;
Word.prototype.draw = function(){
  noStroke();
  fill(red(this.c), green(this.c), blue(this.c), this.a);
  text(this.value, this.x, this.y);
  if(!this.fading){
    this.fadeTo(0, animationDuration/2);
  }
}
