...
function setup(){
  ...
  noStroke();
  //fill(255,32);
  rings.push(new Ring(windowWidth * 0.15, 20, color(255,0,0,128)));
  rings.push(new Ring(windowWidth * 0.2, 30, color(255,128,0,128)));
  rings.push(new Ring(windowWidth * 0.25, 40, color(255,255,0,128)));
  rings.push(new Ring(windowWidth * 0.3, 50, color(128,255,0,128)));
  rings.push(new Ring(windowWidth * 0.35, 60, color(0,255,128,128)));
  rings.push(new Ring(windowWidth * 0.4, 70, color(0,255,255,128)));
}
...
function mousePressed(){
  var rot = (mouseY / windowHeight) * TWO_PI;
  var s = ((mouseX / windowWidth) * (maxSize - minSize)) + minSize;
  for(var i=0; i<rings.length; i++){
    rings[i].rotationPast = rings[i].rotation;
    rings[i].rotationTarget = rot;
    rings[i].rotatingDuration = 3000;
    rings[i].rotatingStart = millis();
    for(var n=0; n<rings[i].chunks.length; n++){
      rings[i].chunks[n].sizeTo(s, 3000, 0);
    }
  }
}
...
Ring.prototype = {
  ...
  resize:function(){
    var ts = random(minSize, maxSize);
    var sd = random(1000,2000);
    var dd = random(10,200);
    for(var i=0; i<this.chunks.length; i++){
      this.chunks[i].sizeTo(ts, sd, dd*i);
    }
  },
...
function RingChunk(pos, a, v1, v2, v3, v4, size){
  ...
  this.sizeTarget = random(minSize, maxSize);
  this.sizePast = this.size;
  this.sizing = true;
  this.sizingStart = millis();
  this.sizingDuration = 2000;
}
...
RingChunk.prototype = {
  ...
  handleSizing:function(){
    if(this.sizing){
      ...
      if(p >= 1){
        ...
        this.size = ((this.sizeTarget - this.sizePast) * this.sinProgress(p)) + this.sizePast;
      }
    }
  },
  ...
  sizeTo:function(sizeTarget, sizingDuration, delayDuration){
    this.sizeTarget = sizeTarget;
    this.sizingDuration = sizingDuration;
