...
function setup(){
  ...
  for(var i=0; i<blobCount; i++){
    blobs.push(new Blob(random(windowWidth), random(windowHeight/2), windowWidth/3, color(255,0,0)));
    blobs.push(new Blob(random(windowWidth), random(windowHeight/2), windowWidth/3, color(0,255,0)));
    blobs.push(new Blob(random(windowWidth), random(windowHeight/2), windowWidth/3, color(0,0,255)));
  }
  noStroke();
  fill(0);
  rectMode(CENTER);
...
Blob.prototype = {
  ...
  draw:function(){
    ...
    //   this.points[i].draw();
    // }
    // recalculate blob center based on point positions
    var avgX = 0;
    var avgY = 0;
    for(var i=0; i<this.points.length; i++){
