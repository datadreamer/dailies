...
function draw(){
  ...
  if(millis() - lastRelease > releaseRate){
    ...
    var c = color(255);
    particles.push(new Particle(random(windowWidth), windowHeight+20, c));
    lastRelease = millis();
  }
...
Particle.prototype = {
  ...
  move:function(){
    ...
    if(dist < pushRadius){
      var force = 1 - dist/pushRadius;
      this.xv -= xdiff * force * forceMult;
      this.yv -= ydiff * force * forceMult;
    }

    // noise
    this.xv += random(-0.2,0.2);
    this.yv += random(-0.3,0.1);
    // measure speed
